...
CEF can also integrate with an existing application message loop instead of running its own message loop. There are two ways to do this.
  ...
  1. Call CefDoMessageLoopWork() on a regular basis instead of calling CefRunMessageLoop(). Each call to CefDoMessageLoopWork() will perform a single iteration of the CEF message loop. Caution should be used with this approach. Calling the method too infrequently will starve the CEF message loop and negatively impact browser performance. Calling the method too frequently will negatively impact CPU usage. On Linux headless/Xvfb, fixed-interval polling is not recommended for production use because it can be both wasteful and vulnerable to starvation if it does not honor [CefBrowserProcessHandler::OnScheduleMessagePumpWork](https://cef-builds.spotifycdn.com/docs/stable.html?classCefBrowserProcessHandler.html#a7ff7d1618399ede096ba16486a71d76e). Prefer the supported Linux external-pump helper in `include/wrapper/cef_linux_external_message_pump.h`, or an equivalent host loop integration that uses the same scheduling contract. You can test this mode in cefclient by running with the "--external-message-pump" command-line flag.
