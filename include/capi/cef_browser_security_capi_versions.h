// Copyright (c) 2026 The Chromium Embedded Framework Authors. All rights
// reserved. Use of this source code is governed by a BSD-style license that
// can be found in the LICENSE file.
//
// ---------------------------------------------------------------------------
//
// This file was generated by the CEF translator tool and should not edited
// by hand. See the translator.README.txt file in the tools directory for
// more information.
//
// $hash=7e9adee5231b60fc7cff1673de1661a7d6e8771e$
//

#ifndef CEF_INCLUDE_CAPI_CEF_BROWSER_SECURITY_CAPI_VERSIONS_H_
#define CEF_INCLUDE_CAPI_CEF_BROWSER_SECURITY_CAPI_VERSIONS_H_
#pragma once

#if !defined(BUILDING_CEF_SHARED)
#error This file can be included DLL-side only
#endif

#include "include/capi/cef_base_capi.h"
#include "include/capi/cef_callback_capi_versions.h"

#ifdef __cplusplus
extern "C" {
#endif

typedef struct _cef_browser_security_callback_0_t {
  cef_base_ref_counted_t base;
  void (CEF_CALLBACK *on_complete)(struct _cef_browser_security_callback_0_t* self, int success, const cef_string_t* message);
} cef_browser_security_callback_0_t;

typedef struct _cef_browser_security_policy_0_t {
  cef_base_ref_counted_t base;
  void (CEF_CALLBACK *get_settings)(struct _cef_browser_security_policy_0_t* self, struct _cef_browser_security_settings_t* settings);
  void (CEF_CALLBACK *set_settings)(struct _cef_browser_security_policy_0_t* self, const struct _cef_browser_security_settings_t* settings, struct _cef_browser_security_callback_0_t* callback);
  cef_string_userfree_t (CEF_CALLBACK *get_allowed_domains)(struct _cef_browser_security_policy_0_t* self);
  cef_string_userfree_t (CEF_CALLBACK *get_confirm_action_categories)(struct _cef_browser_security_policy_0_t* self);
  int (CEF_CALLBACK *should_block_request)(struct _cef_browser_security_policy_0_t* self, const cef_string_t* url);
} cef_browser_security_policy_0_t;

#ifdef __cplusplus
}
#endif

#endif  // CEF_INCLUDE_CAPI_CEF_BROWSER_SECURITY_CAPI_VERSIONS_H_
